...
export class DxfScene3D {
  ...
  *_DecomposeLine(entity, blockCtx) {
  ...
  _GenerateArcVertices({
    ...
    for (let i = 0; i <= numSegments; i++) {
      ...
      vertices.push(new Vector3(v.x, v.y, center.z));
    }
  }
  ...
  *_DecomposePoint(entity, blockCtx) {
    ...
    if (isShaped) {
      ...
      const transformed = this._TransformVertex(entity.position);
      batch.PushVertex(transformed);
  ...
  *_DecomposeFace(entity, vertices, blockCtx, wireframe, transform = null) {
    ...
    if (wireframe) {
      ...
      if (hasFirstTriangle && !hasSecondTriangle) {
        _vertices.push(
          new Vector3(v0.x, v0.y, vertices[0].z),
          new Vector3(v1.x, v1.y, vertices[1].z),
          new Vector3(v2.x, v2.y, vertices[2].z),
        );
      }
      if (!hasFirstTriangle && hasSecondTriangle) {
        _vertices.push(
          new Vector3(v1.x, v1.y, vertices[1].z),
          new Vector3(v3.x, v3.y, vertices[3].z),
          new Vector3(v2.x, v2.y, vertices[2].z),
        );
      } else {
        _vertices.push(
  ...
  *_DecomposePolyline(entity, blockCtx = null) {
    ...
    if (entity.type === 'LWPOLYLINE') {
      ...
      for (let i = 0; i < entity.vertices.length; i++) {
        ...
        vertices.push({ x: v.x, y: v.y, z: elevation });
      }
  ...
  *_DecomposeSpline(entity, blockCtx = null) {
  ...
  _ProcessPolyline(entity, blockCtx = null) {
    // console.log('_ProcessPolyline', entity.vertices)
    if (entity.vertices.length < 2) {
      return;
    }
  ...
  _TransformVertex(v, blockCtx = null) {
    ...
    const res = { x: v.x - this.origin.x, y: v.y - this.origin.y, z: v.z ?? 0 };
...
class RenderBatch {
  ...
  PushChunk(verticesCount) {
    if (verticesCount > INDEXED_CHUNK_SIZE) {
      ...
      console.log('Vertices count exceeds chunk limit: ' + verticesCount);
    }
    ...
    if (this.key.IsIndexed()) {
      ...
      for (const chunk of batch.chunks) {
        ...
        for (let i = 0; i < verticesSize; i += 3) {
          ...
          chunkWriter.PushVertex(new Vector3(v.x, v.y, chunk.vertices.Get(i + 2)));
        }
      ...
      for (let i = 0; i < n; i += 3) {
        ...
        this.PushVertex(new Vector3(v.x, v.y, batch.vertices.Get(i + 2)));
      }
    }
  }
...
class BlockContext {
  ...
  TransformVertex(v) {
    ...
    if (this.block.offset === null) {
      ...
      this.block.UpdateBounds(v0);
      return new Vector3(v0.x, v0.y, v.z ?? 0);
    }
    result.sub(this.block.offset);
    this.block.UpdateBounds(result);
    return new Vector3(result.x, result.y, v.z ?? 0);
  }
...
class IndexedChunkWriter {
  ...
  Finish() {
    if (this.numVerticesPushed !== this.verticesCount) {
      ...
      console.log(`Not all vertices pushed: ${this.numVerticesPushed}/${this.verticesCount}`);
    }
  }
}

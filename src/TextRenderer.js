...
export class TextRenderer {
  ...
  *RenderMText({
    ...
    yield* box.Render(position, width, rotation, direction, attachment, lineSpacing, color, layer);
  }
        ...
        for (
          ...
          if (chunk.block) {
            yield* chunk.block.Render(
              new Vector3(v.x, v.y, position.z),
              null,
              rotation,
              null,
              HAlign.LEFT,
...
TextBox.Paragraph = class {
  ...
  BuildLines(boxWidth) {
    ...
    const CommitLine = () => {
      this.lines.push(
        new TextBox.Paragraph.Line(this, startChunkIdx, curChunkIdx - startChunkIdx, curWidth),
      );
...
class TextBlock {
  ...
  *Render(startPos, endPos, rotation, widthFactor, hAlign, vAlign, color, layer) {
    ...
    for (const glyph of this.glyphs) {
      if (glyph.vertices) {
        for (let i = 0; i < glyph.vertices.length; i++) {
          const v0 = glyph.vertices[i];
          v0.applyMatrix3(transform);
          glyph.vertices[i] = new Vector3(v0.x, v0.y, startPos.z);
        }
